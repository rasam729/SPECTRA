...
THRESHOLDS = {
    ...
    "distance_cm": {
        "warn":     40,    # within 40 cm → proximity warning
        "critical": 20,    # within 20 cm → intruder / ALERT  (dist > 0 && dist < 20)
        "states":   ["INTRUSION"],
    },
}
...
VIB_PATTERNS = {
    ...
    "INTRUSION": [150, 50, 150, 50, 400],  # rapid short pulses for proximity
}
...
class AlertStateMachine:
    ...
    def evaluate(self, packet: dict) -> dict | None:
        ...
        with self._lock:
            ...
            # --- Distance (HC-SR04) ---
            dist = packet.get("distance_cm", -1)
            if dist is not None and 0 < dist < THRESHOLDS["distance_cm"]["critical"]:
                triggered_states.append("INTRUSION")
                triggered_fields.append("distance_cm")
                severity = "CRITICAL"
            elif dist is not None and 0 < dist < THRESHOLDS["distance_cm"]["warn"]:
                triggered_states.append("INTRUSION")
                triggered_fields.append("distance_cm")

            # --- Proximity flag from firmware ---
            if packet.get("proximity_alert"):
                triggered_states.append("INTRUSION")
                if "distance_cm" not in triggered_fields:
                    triggered_fields.append("proximity_alert")
    ...
    def _build_message(
        ...
        msgs = {
            ...
            "INTRUSION":  f"[FIRE_HAZARD] Zone {zone} — PROXIMITY ALERT. Object/person detected within {packet.get('distance_cm','?')}cm. LED: RED.",
...
@sio.on("manual_override")
def on_manual_override(data):
    """
    PWA → Bridge → ESP32 state override.
    Payload: { "state": "SECURE"|"ALERT"|"SOS", "zone": "A" }
    Writes a JSON command line to the ESP32 serial port so it can
    update its LED state machine without waiting for sensor escalation.
    In demo mode: echoes the command back as a synthetic sensor packet.
    """
    state   = data.get("state", "SECURE").upper()
    zone    = data.get("zone",  "A")
    _log(f"[Override] PWA override → ESP32: state={state}, zone={zone}")

    # Broadcast acknowledgement back to all dashboard clients
    sio.emit("override_ack", {
        "state":     state,
        "zone":      zone,
        "source":    "PWA_OVERRIDE",
        "timestamp": _utcnow(),
    ...
    # Write command to serial (if real hardware is connected)
    cmd = json.dumps({"override": state, "zone": zone}) + "\n"
    try:
        # Find the active serial reader's port and write to it
        import serial as _serial
        # Attempt to open same port used at startup (best-effort, no-op in demo)
        # The ESP32 firmware should handle {"override":"ALERT"} JSON on Serial input
        for port in ["COM3", "/dev/ttyUSB0", "/dev/ttyACM0"]:
            try:
                s = _serial.Serial(port, 115200, timeout=0.1)
                s.write(cmd.encode())
                s.close()
                _log(f"[Override] Written to {port}: {cmd.strip()}")
                break
            except Exception:
                continue
    except ImportError:
        pass  # pyserial not available — demo mode, command logged only

    # Synthetic sensor packet so dashboard state machine reflects override
    synthetic = {
        "zone":          zone,
        "co2":           350.0 if state == "SECURE" else 2500.0,
        "temp_c":        25.0  if state == "SECURE" else 480.0,
        "smoke_ppm":     30.0  if state == "SECURE" else 900.0,
        "heartrate_bpm": 72,
        "distance_cm":   100  if state == "SECURE" else 10,
        "led_state":     state,
        "sos_flag":      state == "SOS",
        "override":      True,
        "timestamp":     _utcnow(),
